...
void UpdateTC( inout p_bumped I)
{
	...
	float2	new_tc  = I.tcdh.xy + height * normalize(eye).xy;	//
...
surface_bumped sload_i( p_bumped I)
{
	...
	float4 	Nu	= tex2D( s_bump, I.tcdh.xy );		// IN:	normal.gloss
	float4 	NuE	= tex2D( s_bumpX, I.tcdh.xy);		// IN:	normal_error.height
